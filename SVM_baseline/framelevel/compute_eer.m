function [EER, prec, recall, F1] = compute_eer(result_filename, label, label_assignments, label_assignments_filelist)
...
[fpr, tpr, T] = perfcurve(label_assignments, results.scores, 1);
...
Pos_t = find(results.scores > T(i));
Pos_l = find(label_assignments == 1);
L_pos = length(Pos_t);
L_l = length(Pos_l);
